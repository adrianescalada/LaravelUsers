...
[![Latest Stable Version](https://poser.pugx.org/wdna/api-guard/v/stable)](https://packagist.org/packages/wdna/api-guard) [![Total Downloads](https://poser.pugx.org/wdna/api-guard/downloads)](https://packagist.org/packages/wdna/api-guard)

[![Join the chat at https://gitter.im/wdna/api-guard](https://badges.gitter.im/Join%20Chat.svg)](https://gitter.im/wdna/api-guard?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge)
...
**Laravel 5.1.x to 5.2.x: [`~3.*`](https://github.com/wdna/api-guard/blob/3.1/README.md)

**Laravel 5.1.x: `~2.*`

**Laravel 4.2.x: [`~1.*`](https://github.com/wdna/api-guard/tree/laravel4) (Recently updated version for Laravel 4. Please note that there are namespace changes here)

**Laravel 4.2.x: [`0.*`](https://github.com/wdna/api-guard/tree/v0.7) (The version that most of you are using)

## Quick start

### Installation for Laravel 5.3 to 5.4

Run `composer require wdna/api-guard 4.*`

In your `config/app.php` add `wdna\ApiGuard\Providers\ApiGuardServiceProvider` to the end of the `providers` array

```php
'providers' => array(

    ...
    wdna\ApiGuard\Providers\ApiGuardServiceProvider::class,
),
```

Now publish the migration and configuration files for api-guard:

    $ php artisan vendor:publish --provider="wdna\ApiGuard\Providers\ApiGuardServiceProvider"
...
```php
use wdna\ApiGuard\Models\Mixins\Apikeyable;
...
```php
$apiKey = wdna\ApiGuard\Models\ApiKey::make()

// Attach a model to the API key
$apiKey = wdna\ApiGuard\Models\ApiKey::make($model)
```
...
```php
use wdna\ApiGuard\Http\Controllers\ApiGuardController;
...
```php
use wdna\ApiGuard\Http\Requests\ApiGuardFormRequest;
